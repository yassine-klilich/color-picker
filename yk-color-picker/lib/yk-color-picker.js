...
const _gui_ = {
  initDOM: function () {
    // _dom declaration
    const cp_overlayWrapper = _gui_.createElement("div", [
      "cp-overlay-wrapper",
    ]);
  ...
  buildColorInputs: function () {
    // Create elements
    const inputsSettings = _gui_.createElement("div", [
      "cp-color-model-wrapper",
    ]);
    const inputsWrapper = _gui_.createElement("div", ["cp-color-model"]);
    const inputsSwitch = _gui_.createElement("button", [
    ...
    inputsSwitch.addEventListener(
      "click",
      _event_.onClickInputsSwitch.bind(this)
    );
  ...
  buildHueSlider: function () {
    ...
    sliderThumb.addEventListener(
      "keydown",
      _event_.onKeyDownHueSlider.bind(this)
    );
  ...
  buildOpacitySlider: function () {
    // Create elements
    const sliderWrapper = _gui_.createElement("div", [
    ...
    this.__onMouseDownOpacitySlider =
      _event_.onMouseDownOpacitySlider.bind(this);
    this.__onMouseUpOpacitySlider = _event_.onMouseUpOpacitySlider.bind(this);
    this.__onMouseMoveOpacitySlider =
      _event_.onMouseMoveOpacitySlider.bind(this);
    ...
    sliderThumb.addEventListener(
      "keydown",
      _event_.onKeyDownOpacitySlider.bind(this)
    );
  ...
  buildColorPreview: function () {
    const colorPreviewWrapper = _gui_.createElement("span", [
  ...
  updateHEXColorSection: function (
    color,
    event,
    con,
    conValue,
    ...
    if (con(rgb[color], conValue)) {
      ...
      this.__c.hex =
        ...
        _util_.hexPad2(Math.round(rgb[color])) +
  ...
  updateHEXAlphaSection: function (event, con, conValue, op) {
    ...
    if (con(a, conValue)) {
      ...
      target.value = this.__c.hex =
        hex.substring(0, 7) + _util_.hexPad2(Math.round(this.__c.a * 255));
      _gui_.updateColorPreview.call(this, true);
      _gui_.updateOpacityThumb.call(this);
    }
    target.value = this.__c.hex;
...
const _event_ = {
  ...
  onMouseMoveCursor: function (event) {
    const { x, y } = _core_.getCursorPosition.call(
      this,
      event.clientX,
      event.clientY
    );
      ...
      case "ArrowUp":
        {
          ...
          if (length <= 5) {
            if (caret < 2) {
              _gui_.updateHEXColorSection.call(
                this,
                "r",
                event,
                _util_.lt,
              ...
              _gui_.updateHEXAlphaSection.call(
                this,
                event,
                _util_.lt,
                1,
  ...
  onClickCopyColor: function () {
    navigator.clipboard.writeText(_core_.getColorText.call(this)).then(() => {
      this._dom.copyColor.style.setProperty(
        "background-image",
      ...
      case "ArrowRight":
        {
          ...
          if (!isNaN(position) && position < opacitySlider.offsetWidth) {
            ...
            _gui_.updateOpacityValue.call(
              this,
              position / opacitySlider.offsetWidth
            );
          }
          event.preventDefault();
        }
