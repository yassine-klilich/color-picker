...
function _buildHEXInput() {
  ...
  inputHEX.addEventListener("focus", _onFocusInput.bind(this))
  inputHEX.addEventListener("keydown", _onKeyDownInputHEX.bind(this))
  inputHEX.addEventListener("input", _onInputHEX.bind(this))
  inputHEX.addEventListener("change", _onChangeInputHEX.bind(this))
...
function _buildQuadrupedInput() {
  ...
  inputA.addEventListener("input", _onInputA.bind(this))
  ...
  inputB.addEventListener("input", _onInputB.bind(this))
...
function _onKeyDownInputHEX(event) {
  const { target, key } = event
  switch (key) {
    case "ArrowUp": {
      ...
      const caret = _getCaretPosition(target)
      const length = target.value.length
      if (length <= 5) {
        if (caret < 2) {
          _updateHEXColorSection.call(this, 'r', event, _lt, 255, _add, 1, 3)
        }
        else if (caret < 3) {
          _updateHEXColorSection.call(this, 'g', event, _lt, 255, _add, 3, 5)
        }
        else if ((caret <= 4 && length <= 4) || caret < 4) {
          _updateHEXColorSection.call(this, 'b', event, _lt, 255, _add, 5, 7)
        }
        else if (caret <= 5) {
          _updateHEXAlphaSection.call(this, event, _lt, 1, _add)
        }
      }
      else {
        if (caret < 3) {
    ...
    case "ArrowDown": {
      ...
      if (length <= 5) {
        if (caret < 2) {
          _updateHEXColorSection.call(this, 'r', event, _gt, 0, _sub, 1, 3)
        }
        else if (caret < 3) {
          _updateHEXColorSection.call(this, 'g', event, _gt, 0, _sub, 3, 5)
        }
        else if ((caret <= 4 && length <= 4) || caret < 4) {
          _updateHEXColorSection.call(this, 'b', event, _gt, 0, _sub, 5, 7)
        }
        else if (caret <= 5) {
          _updateHEXAlphaSection.call(this, event, _gt, 0, _sub)
        }
      }
      else {
        if (caret < 3) {
...
function _onInputHEX(event) {
  ...
  if (/^#([0-9a-f]{3}|[0-9a-f]{4}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(value)) {
    ...
    _updateColorView.call(this)
    _updateHueThumb.call(this)
    _updateOpacityThumb.call(this)
    _updateCursorPalette.call(this)
  }
}

function _onChangeInputHEX(event) {
...
function _buildCopyColor() {
  const copyColorWrapper = _createElement("span")
  const copyColor = _createElement("button", ["cp-clipboard-color"])
  const hiddenInput = _createElement("input")
...
function _rgbUpdateView() {
  _updateColorView.call(this)
  _updateHueThumb.call(this)
  _updateCursorPalette.call(this)
}

function _updateHEXColorSection(color, event, con, conValue, op, startSelect, endSelect) {
  const { target } = event
...
function _updateHEXAlphaSection(event, con, conValue, op) {
  const { target } = event
...
function _getCaretPosition(target) {
