    ...
    case "ArrowUp": {
      ...
      if (valueLength <= 5) {
        if (caretPosition < 2) {
          ...
          if (rgb.r < 255) {
            ++rgb.r
            target.value = this.color.hex = hex.substring(0, 1) + rgb.r.toString(16).padStart(2, '0') + hex.substring(3)
          }
          ...
          if (rgb.g < 255) {
            ++rgb.g
            target.value = this.color.hex = hex.substring(0, 3) + rgb.g.toString(16).padStart(2, '0') + hex.substring(5)
          }
          target.setSelectionRange(3, 5)
          event.preventDefault()
        }
        else if ((caretPosition <= 4 && valueLength <= 4) || caretPosition < 4) {
          const { rgb, hex } = this.color
          if (rgb.b < 255) {
            ++rgb.b
            target.value = this.color.hex = hex.substring(0, 5) + rgb.b.toString(16).padStart(2, '0') + hex.substring(7)
          }
          target.setSelectionRange(5, 7)
          event.preventDefault()
        }
        else if (caretPosition <= 5) {
          let { hex, a } = this.color
          if (a < 1) {
            this.color.a = parseFloat((a + 0.01).toFixed(2))
        ...
        else if ((caretPosition <= 7 && valueLength == 7) || caretPosition < 7) {
          const { rgb, hex } = this.color
          if (rgb.b < 255) {
            ++rgb.b
            target.value = this.color.hex = hex.substring(0, 5) + rgb.b.toString(16).padStart(2, '0') + hex.substring(7)
