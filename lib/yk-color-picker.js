...
const _core_ = {
  ...
  updateHEXColor: function() {
    ...
    let hex = this.__c.RGBtoHEX(r, g, b)
    if (this.__c.a < 1) {
      hex += Math.round(this.__c.a * 255).toString(16).padStart(2, '0')
    }
    this.__c.hex = hex
  },
...
const _gui_ = {
  ...
  updateOpacityThumb: function() {
    ...
    opacityThumb.style.setProperty("transform", `translate(${(this.__c.a * opacitySlider.offsetWidth) - (opacityThumb.offsetWidth / 2)}px, -50%)`)
  },
  updateHueThumb: function() {
    const { hueThumb, hueSlider } = this.DOM
    hueThumb.style.setProperty("transform", `translate(${((this.__c.hsv.h / 360) * hueSlider.offsetWidth) - (hueThumb.offsetWidth / 2)}px, -50%)`);
  },
...
const _event_ = {
  ...
  onInputHEX: function(event) {
    ...
    if (/^#([0-9a-f]{3}|[0-9a-f]{4}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(value)) {
      const { r, g, b, a  } = this.__c.HEXtoRGB(value)
      this.__c.a = a
      this.__c.rgb = { r, g, b }
      let hex = this.__c.RGBtoHEX(r, g, b)
      if (this.__c.a < 1) {
  ...
  onMouseMoveHueSlider: function(event) {
    ...
    this.__c.hsv.h = Math.round(((thumbX + sliderThumbHalfWidth) / sliderRect.width) * 360)
  ...
  onMouseMoveOpacitySlider: function(event) {
    ...
    this.__c.a = parseFloat(((thumbX + opacityThumbHalfWidth) / opacitySliderRect.width).toFixed(2))
