...
function _extractHSVColor(x, y) {
  ...
  this.color.s = Math.round((x / paletteWidth) * 100)
  this.color.v = Math.round(((paletteHeight - y) / paletteHeight) * 100)
}
      ...
      case "ArrowUp": {
        switch (this.currentRepresentation) {
          ...
          case HSL: {
            ...
            let hsl_s = s
            if (s < 100) {
              let { s } = this.color.HSLtoHSV(this.color.h, hsl_s, l)
              target.value = ++hsl_s + "%"
              this.color.s = s
              _updateColorView.call(this)
            }
